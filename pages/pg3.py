...
dash.register_page(__name__,
                   path='/violencia-sexual-domestica',  # represents the url text
                   name='Porcentaje de mujeres que han experimentado violencia sexual doméstica',  # name of page, commonly used as name of link
                   title='Porcentaje de mujeres que han experimentado violencia sexual doméstica'  # epresents the title of browser's tab
)
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Mujeres', id='page3-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
